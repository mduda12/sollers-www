<?php
    ...
    if( !class_exists( '\Wpo\Aad\Auth' ) ) {
        ...
        class Auth {
            ...
            // Used by AAD v2.0 - app only token
            ...
            public static function validate_current_session() {
                ...
                if( !empty( $_REQUEST[ 'action' ] ) && $_REQUEST[ 'action' ] == 'openidredirect' ) {
                    ...
                    if( Helpers::is_wpo365_configured() ) {
                ...
                if( Options::get_global_boolean_var( 'test_mode' ) 
                    && !( is_admin() && defined( 'DOING_AJAX' ) && DOING_AJAX ) ) {
                        // Perform a self test
                        require_once( $GLOBALS[ 'WPO365_PLUGIN_DIR' ] . '/Wpo/Tests/Self_Test.php' );
                        new \Wpo\Tests\Self_Test();

                        // Immediately disable test mode
                        Options::update_options( array(
                            'test_mode' => false,
                        ), true );
                        ...
                        $redirect_url = Helpers::get_redirect_url( $redirect_to );
            ...
            public static function process_openidconnect_token() {
                ...
                // User is ensured and we now have a WordPress user -> time to save prefetched access / refresh token

                $request = Request::get_instance( $_GET[ 'WPO365_REQUEST_ID' ] );
                $access_token = $request->get_item( 'prefetched_access_token' );
                ...
                if( false !== $access_token ) {
                    ...
                    // Save refresh token
                    if( property_exists( $access_token, 'refresh_token' ) && isset( $access_token->refresh_token ) ) {
                        self::set_refresh_token( $access_token );
                    }
                }
                ...
                if( !empty( $cached_access_tokens_json ) ) {
                    ...
                    foreach( $cached_access_tokens as $key => $cached_access_token ) {
                        ...
                        elseif( isset( $cached_access_token->scope ) && false !== stripos( $cached_access_token->scope, $scope ) ) {
                            Logger::write_log( 'DEBUG', 'Found a previously saved access token for ( ' . $scope . ' ) ' . $cached_access_token->scope . ' that is still valid' );
             ...
             * @return mixed(stdClass|WP_Error) access token as object or WP_Error
             */
            public static function prefetch_bearer_token( $scope ) {

                if( !isset( $_POST[ 'code' ] ) || empty( $_POST[ 'code' ] ) ) {
                    $error_message = 'Authorization code not present when trying to prefetch access code.';
                    Logger::write_log( 'ERROR', $error_message );
                    return new \WP_Error( '1041', $error_message );
                }
                ...
                Logger::write_log( 'DEBUG', 'Prefetching access token for ' . $scope );
                ...
                if( curl_error( $curl ) ) {
                    $error_message = 'Error occured whilst prefetching an access token';
                    Logger::write_log( 'ERROR', $error_message );
                    curl_close( $curl );
                ...
                if( is_wp_error( $access_token ) ) {
                    Logger::write_log( 'ERROR', 'Prefetched access token for ' . $scope . ' appears to be invalid' );
                    return new \WP_Error( $access_token->get_error_code(), $access_token->get_error_message() );
                }
                ...
                Logger::write_log( 'DEBUG', 'Successfully prefetched a valid access token for ' . $scope );
                
                $request = Request::get_instance( $_GET[ 'WPO365_REQUEST_ID' ] );
                $request->set_item( 'prefetched_access_token', $access_token );

                // Remove the code so it isn't stored further down the line
                unset( $_POST[ 'code' ] );
             ...
             * @return mixed(stdClass|WP_Error) access token as object or WP_Error
             */
            public static function get_app_only_bearer_token() {
                ...
                if( !empty( $cached_access_token_json ) ) {
                    ...
                    if( empty( $cached_access_token ) ) {
                        ...
                        Logger::write_log( 'DEBUG', 'Deleted an invalid app-only access token.' );
                    }
                    ...
                    if( isset( $cached_access_token->expiry ) && intval( $cached_access_token->expiry ) < time() ) {
                        ...
                        Logger::write_log( 'DEBUG', 'Deleted an expired app-only access token.' );
                    }

                    // Valid app only token found
                    if( !empty( $cached_access_token ) ) {
                        Logger::write_log( 'DEBUG', 'Found a previously saved app-only access token that is still valid' );
                        return $cached_access_token;
                    }
                }

                $params = array(
                    'client_id' => Options::get_global_string_var( 'app_only_application_id' ),
                    'client_secret' => Options::get_global_string_var( 'app_only_application_secret' ),
                    'grant_type' => 'client_credentials',
                    'scope' => 'https://graph.microsoft.com/.default',
                );

                Logger::write_log( 'DEBUG', 'Requesting app-only access token' );
                ...
                if( curl_error( $curl ) ) {
                    $error_message = 'Error occured whilst getting an app-only access token';
                    Logger::write_log( 'ERROR', $error_message );
                    curl_close( $curl );
                ...
                if( is_wp_error( $access_token ) ) {
                    Logger::write_log( 'ERROR', 'App-only access token appears to be invalid' );
                    return new \WP_Error( $access_token->get_error_code(), $access_token->get_error_message() );
                }
                ...
                Logger::write_log( 'DEBUG', 'Successfully obtained a valid app-only access token' );
                Logger::write_log( 'DEBUG', $access_token );
