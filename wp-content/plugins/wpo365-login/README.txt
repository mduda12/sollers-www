...
Contributors: wpo365
Tags: office 365, O365, azure active directory, Azure AD, AAD, authentication, single sign-on, sso, login, oauth, microsoft, microsoft graph, sharepoint online, sharepoint, spo, onedrive
...
* v8.0 When the client-side solution (to redirect users) is not working as expected you can enable a fallback that uses server-side redirection by checking the "Use server side redirect" option on the miscellaneous tab of the plugin's wizard. This option, however, does not try and bypass server-side cache and as a result you may start seeing "Your login has been tampered with" errors.
...
There is a [new video](https://youtu.be/DmRZbKGminE) available on YouTube that covers the updated configuration (for v7.18, not for v8.0) in detail.
